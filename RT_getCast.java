...
public class RottenTomatoes
{
    ...
    { 
      ...
      public void generateCastFile(String movie_name) throws Exception
      {
    	String Filename = movie_name + "_cast.txt";
		File inputFile = new File(Filename);
		FileWriter fileWriter;
		//if it fails the below test will return from here and not go further
		//thus no need to put rating retrieval in a if block.
		List<String> cast = get_cast(movie_name);
		if (cast == null)
		{
			return;
		}
    ...
    public static void main(String[] args) throws Exception
    {
    	...
    	for (int i = 0;i<movielist.size();i++)
    	{
    		Thread.sleep(10000);//sleep for 10 seconds, so that API limit is not violated.
    		 ...
    		 System.out.println("Done::: "+  movielist.get(i));
